...
def get_icon_path(icon_filename: str) -> Path:
    if getattr(sys, "frozen", False):
        # Py2app bundle: load from Resources/
        return Path(sys.executable).parent.parent / "Resources" / icon_filename
    else:
        # Dev mode
        return Path(__file__).parent.parent / "assets" / "images" / icon_filename
...
def set_folder_icon(directory_path: str, icon_path: str) -> bool:
    ...
    success = NSWorkspace.sharedWorkspace().setIcon_forFile_options_(
        icon, directory_path, 0
    )
    if success:
